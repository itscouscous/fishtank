...
<body>
    <div class="container">
        ...
        <div class="task-container">
            <button class="task-btn" data-task="1">Remote Control</button>
            <button class="task-btn" data-task="2">Wall Following</button>
            <button class="task-btn" data-task="3">Attack Lower</button>
            <button class="task-btn" data-task="4">Attack Upper</button>
            <button class="task-btn" data-task="5">Attack Base</button>
            <button class="task-btn" data-task="6">Attack All</button>
        </div>
        
        <!-- Arm Control Button -->
        <div class="section-title">Arm Control:</div>
        <div class="task-container">
            <button class="task-btn arm-btn" id="arm-toggle">Activate Arm</button>
        </div>
    ...
    <script>
        ...
        const state = {
            ...
            task: 1,  // Default task: Remote control
            arm: 0    // Default arm state: Deactivated
        };
        ...
        function updateButtonVisuals() {
            ...
            taskButtons.forEach(button => {
                const taskId = parseInt(button.getAttribute('data-task'));
            ...
            if (state.arm === 1) {
                ...
                armButton.textContent = 'Arm Activated';
            } else {
                armButton.classList.remove('active');
                armButton.textContent = 'Activate Arm';
            }
        }
        ...
        // Function to send task to ESP
        function sendTaskToESP(taskId) {
            const command = `task:${taskId}`;
            const xhr = new XMLHttpRequest();
            xhr.open("GET", "/task?val=" + taskId, true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        responseDisplay.textContent = xhr.responseText;
        ...
        // Function to send arm state to ESP
        function sendArmToESP(armState) {
            const command = `arm:${armState}`;
            const xhr = new XMLHttpRequest();
            xhr.open("GET", "/arm?val=" + armState, true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        responseDisplay.textContent = xhr.responseText;
        ...
        taskButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                if (state.task !== taskId) {
                    state.task = taskId;
                    sendTaskToESP(taskId);
                    updateButtonVisuals();
                }
            });
        });
        
        // Arm toggle button event listener
        armButton.addEventListener('click', (event) => {
            state.arm = state.arm === 0 ? 1 : 0;
            sendArmToESP(state.arm);
            updateButtonVisuals();
        });
